...
\solution
\paragraph{Number of nodes.} To find the equation for $N_\mathrm{a} $ we need to work out what proportion of the network is removed in the attack: 
\eq{
r=R(1)=G(1)-G(y) = 1-G(y),  
}
...
\paragraph{Mean degree.} To find the result we use the respective result for degree targeted attacks 
...
In this case we must take care with the derivative as we are differentiating with respect to $x$, which appears as the argument of $\tilde{A}$, (remember $\tilde{A}=\tilde{A}(x)$). We find
\eqa{
...
\paragraph{Excess degree.} As for the mean degree there are two ways to reach the desired result. First, we can use the equation for the excess degree after a degree targeted attack $q_{\rm a}=q-R''(1)/z$. We start by computing the second derivative of $R$,
\eqa{
...
\paragraph{Degree generating function.} Here we use the result from degree-targeted attacks, which tells us
\eqa{ 
...
\paragraph{Excess degree generating function.} Here we have again the choice between two different ways. Using the result from degree-targeted attacks we can write
\eqa{
