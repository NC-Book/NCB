...
\solution 
\paragraph{Network properties.}We follow the procedure established in the chapter and start by computing some basic network properties. We start by writing the degree generating function 
\eq{
...
We can now compute the excess degree generating function 
\eq{
...
\paragraph{Giant conducting component.}Before we dive into the main part of the calculation, first let's quickly check if there is a giant conducting component at all. We compute 
\eq{
...
\eq{
v_{\rm c}= 0.5(0.4v_{\rm c}+0.6)+0.5(0.4v_{\rm c}+0.6)^7
}
...
In step 3 of the viral attack algorithm, we compute the proportion of all links that are not in the giant conducting component
\eq{
...
\paragraph{Conclusion.}This tells us that starting the viral attack in a random node has 55\% chance to cause the removal of 55\% of the network. 

If those 55\% of the nodes were chosen randomly then the mean excess degree after the attack would be $(1-r)q\approx 1.788>1$, but the random attack does not choose its targets randomly. So, instead we compute the mean excess degree after the attack as follows 
\eqa{
q_{\rm a} &=& y^2 Q'(y)\\
  &=& y^2 (0.5+3.5y^6) \\
  &=& 0.5 y^2 + 3.5 y^8 \\
