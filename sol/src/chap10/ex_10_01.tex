\exercise{Polynomial Long Division}{1}\label{exAttackPolyLong}In the chapter we needed to solve $v=(0.8v+0.2)^3$. Use the known solution $v=1$ to reduce this equation to a quadratic form by polynomial long division. [Check the solution, if you haven't seen this before.]

\solution
\paragraph{Bringing the condition into a nicer form.}To get started we bring the condition into a nicer form. Write the decimals as fractions, multiply out the cubic function, and bring everything to one side. 
\eqa{
...
\eq{X=112v+Y,}
where $Y$ is an undetermined constant. Substituting this into our previous condition we find we have
\eqa{
(112v+Y)(v-1)&=&112v^2-113v+1\\
112v^2-112v+Y(v-1)&=&112v^2-113v+1\\
Y(v-1)&=&-v+1\\
Y=-1
}
\paragraph{Solution.}We can now put the solution together from our equations above. The quadratic polynomial we are looking for is  
\eq{
64v^2+X=64v^2+112v+Y=64v^2+112v-1
}
\paragraph{Summary.}In summary we have shown
\eq{
