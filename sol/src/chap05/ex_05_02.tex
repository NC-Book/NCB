...
\paragraph{Part (a).}To solve (a) we use
\eq{
x=\log_{10}(20) = \log_{10}(2)+\log_{10}(10) \approx 0.3 + 1 = 1.3
}
Note that the second term, where we used rule ii, and then rule vi.\\[0.5em] 
\paragraph{Part (b).}For part (b) we can do the following:
\eq{
x=\log_{10}(4000)=\log_{10}(4)+\log_{10}(1000) = \log_{10}(2^2)+\log_{10}(10^3)\approx 2\cdot 0.3+3 \approx 3.6 
}
where we used rule ii, then wrote the numbers as powers to apply rule iii.\\[0.5em]
\paragraph{Part (c):} To solve (c), we need rule iv
\eq{
x=\log_{1000}(2)=\frac{\log_{10}(2)}{\log_{10}(1000)}\approx \frac{0.3}{3}=0.1 
}\\[0.5em]
\paragraph{Part (d).} We do part (d) almost in the same way
\eq{
...
\\[0.5em]
\paragraph{Part (e).} Finally for (e) we start with a change of basis (rule iv)
\eq{
