...
\solution 
In the logistic differential equation the variable is called $x$ while in the SIS model we called it $I$, so let's start by renaming, $I=x$. In terms of $x$ we can write our SIS model as 
\eq{
\dot{x}=p(1-x)x -rx
}
To bring it into the form of the logistic differential equation, it is useful to first collect all powers of $x$,
\eq{
\dot{x} = (p-r)x - p x^2  
}
...
This condition must be fulfilled for all values of $x$, which is only possible if we choose $a=b$. From the first bracket we see furthermore that $a=1/K$.
...
\eq{
\frac{{\rm d}}{{\rm d}x} \log(K-x) = \frac{-1}{K-x} 
}
where the $-1$ appears due to the inner derivative. If we multiply the whole equation and multiply by $-1$ and then integrate it, again we get 
\eq{
...
We now need to solve for $x$, to do so, we first multiply everything by $K$,
\eq{
