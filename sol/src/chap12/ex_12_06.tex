...
\subquestion Find the general solution to the differential equation. [Hint: If stuck, consider $\delta=x^*-x$.]
...
The problem with this solution is that you need to be able to integrate $1/(a-bx)$. This is not an insurmountable hurdle as you might just remember this integral, find it by trial and error (we get fractions from differentiating logs so differentiating some promising logs quickly leads to the right solution), or you could just look it up in a table of known integrals (you find them online these days, but having an integral table as a printed book is still valuable). 
...
to also remove the $x$ from the right hand side we use the definition of $\delta$ in the form $x=x^*-\delta$ which yields 
\eq{
...
where $r$ is again an arbitrary constant. To find a solution in terms of $x$ we use the definition of $\delta$ again in the form $x=x^*-\delta$ which yields 
\eq{
x(t) = \frac{a}{b} - r\exp{-bt},
}
