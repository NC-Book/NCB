...
In this chapter we derived the formula $n_{\rm motif} = z^k N^{n-k}/s$ for the approximate number of copies of a motif with 
...
\paragraph{Symmetry number 4-cycle.}Now we have to find the symmetry number $s$. So, we find this number by asking how many different ways are there in which we can assign numbers (1,2,3,4) to the nodes in the four cycle such that we end up with exactly the same links. 
...
\paragraph{Approximated number of copies 4-cycle.}So the full formula is 
\eq{
n_{\mbox{4-cycle}} = \frac{z^4}{8}=1250.
}
\paragraph{Symmetry number $n$-cycle.}In case you still have doubts that the correct number is 8, here are 8 cycles that all use the same links,
\begin{center}
...
\paragraph{Approximated number of copies $n$-cycle.}Finally you may wonder if we can actually use the approximate formula in this case. For $N=100.000$ and $n=4$ the approximation we have made is 
\eq{
