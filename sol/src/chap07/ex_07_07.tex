...
We know that large random graphs with $z\ll N$ are locally treelike, but we also know that if $z>1$ then there must be some cycles. To explore this, consider an ER graph with $N=101$ nodes and mean degree $z=2$. Find out what the most common length $l$ for a cycle is. 

\solution 
So we expect that there are few very short cycles because the network is locally tree-like. We also expect that there are few very long cycles because the network isn't actually that large. So there must be some kind of optimum in the middle. 
\paragraph{Approximate formula.}For a first try we can use the approximate formula 
\eq{
...
This is a weird result. If we increase $l$ we find ever more numerous cycles. But this makes no sense because there can't be any cycles that have a length greater than 101. 
\paragraph{Statistically exact formula.} OK, here is an idea. The error of the approximate formula grows for large motifs. Likely this makes our estimates for long cycles inaccurate. So let's use the statistically exact formula instead.  
\eq{
...
so that's just 1 or two three-cycles. However, maybe we have learned something from this calculation. 
\paragraph{Finding the most common cycle.}Notice that the $n!$ enters from the denominator of the Binomial Formula will always cancel with the $n!$ that is in the front of the motif equation. we can thus use
\eq{
n_{\mbox{$l$-cycle}} = \frac{1}{2l} p^l 101\cdot 100 \cdots 101-l
}
Now consider what happens when we increase $l$ we multiply one more factor of $p$ the denominator of the fraction in the beginning increases by 2 and we multiply a factor of $101-l$ in the end. Does this mean there are more or less $l+1$-cycles than $l$-cycles? Let's consider the ratio 
\eq{
...
\paragraph{Additional insights.}OK we are really done with this exercise, but if we wanted to we could write
\eq{
