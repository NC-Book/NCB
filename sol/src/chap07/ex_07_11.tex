...
The following figure shows the probability that a random graph drawn from an ensemble with mean degree $2$ has a realized mean degree $z$.
\begin{center}
\includegraphics[width=0.4\textwidth]{netsdistribution.png}
\end{center}
Here we illustrate the underlying calculations with the example $z=2$.

\subquestion Compute the probability that an individual network drawn from an ensemble with $N=100$ and mean degree 2, actually has a mean degree of $z=2$. 

\solution
\paragraph{Number of links in the network.}Since we are dealing with a network of $N=100$ nodes, mean degree $z=2$ of the network requires that $K=100$ links are placed during the network creation as we know 
\eq{
z=\frac{2K}{N}.
}
...
\paragraph{Placement of links.}This is the number of maximal links that a simple graph of this size could contain, but also the number of trials for placing links in the network creation. In our case of $N=100$ it is
\eq{
K_{\rm max} = \frac{100\cdot 99}{2} = 4950.
}
Each of these trials succeeds and places a link with probability $p$. From this chapter we know that we need to choose 
\eq{
...
\paragraph{Probability for mean degree $z=2$.}Now putting everything together we can write the probability that a network drawn from the ensemble has $K$ links as 
\eq{
  p^{K}q^{K_{\rm max}-K} B(K_{\rm max},K) = \left(\frac{2}{99}\right)^{100}\left(\frac{97}{99}\right)^{4850}\frac{4950!}{100!\cdot 4850!} 
}
...
\paragraph{Simplifying the equation.}Note that our equation describes a binomial distribution. 
