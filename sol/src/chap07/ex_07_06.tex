...
\solution
\paragraph{Solution 1.}There are different ways to find the answer. For example we can think of an isolated pair as a node of degree 1 that links to a node of excess degree 0.
...
\eq{
n_{\mbox{isolated pair}} = \frac{1000\cdot 999}{2} \frac{4}{999} \left( \frac{995}{999} \right)^{1996} \approx 0.7
}
as before.
...
\paragraph{Isolated triangles.}You might simply guess that isolated triangles are super rare in any ER-graph ($N=3$, $p=1$ is perhaps the only exception to this rule) 
...
\paragraph{Isolated 3-chain.}So how about the isolated 3-chain? A quick way to compute their number is to first compute the number of nodes of degree 2
\eq{
