...
\subquestion Consider this walk on the 3-chain, (1)-(2)-(3). Define $x_i$ as the probability that the walker is currently in node $i$. Derive the differential equations that govern these variables and show that they can be written as
\eqn{
...
If the walker is currently in node 1 (probability $x_1$) it leaves at rate 1, leading to a loss rate of $-x_1$ from $x_1$. If the walker is currently in node 2 (probability $x_2$) it leaves also at rate 1 and chooses to go to node 1 with 50\% probability. This results in a gain rate of $x_2/2$ for $x_1$. Using the same reasoning also for the other nodes leads to  
\eqa{
\dot{x}_1 &=& -x_1 + x_2/2 \\
...
where $\delta_{ij}$ is the Kronecker delta. We can now define the term in the braces as $L_{{\rm wr},ij}$, the element $i,j$ of $\bf L_{wr}$. This yields 
\eq{
...
We recognize the equation for $\dot{x}_i$ as the $i$-th row of a matrix vector product. Hence, the equation for the entire product is
\eq{
\dot{\vec{x}} = - {\bf L_{wr}} \vec{x},
}
which is what we wanted. But now let's have a look at the matrix. The Kronecker delta puts the number 1 into the diagonal elements, as an identity matrix would. In fact, we can define the identity matrix by $I_{ij}=\delta_{ij}$. The second term contains $A_{ij}$ which is the adjacency matrix element, but it is divided by the degree of node $j$. In the chapter, we saw that this is what happens when we multiply the inverse of the degree matrix $\bf K$ from the right. So we can write
\eq{
...
but the adjoint or transpose of that matrix. We can show 
\eq{
...
But no, because it doesn't matter. The argument that we used in the chapter to show that we can use the eigenvectors of the symmetric Laplacian holds both ways: The signs of the elements of the right eigenvectors are the same as the signs of the elements of the right eigenvectors of the symmetric matrix. For the symmetric matrix, the right and left eigenvectors are the same. And the signs of the elements of the left eigenvector of the symmetric matrix are the same as the signs of the elements of the left eigenvector of the random walk Laplacian. 
