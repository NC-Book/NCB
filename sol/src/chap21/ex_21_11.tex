...
\subquestion 
Study this process first on the 3-chain, (1)-(2)-(3). Write a discrete time map that captures the changes of the probabilities, then solve it for a walker starting in node $1$ at $t=0$. 
...
which we recognize from the chapter. To solve the dynamics, we need the eigenvectors. Based on the network's symmetry, we guess that 
\eq{
\vec{v_1}  = \avec{1 \\ 0 \\ -1} 
}
