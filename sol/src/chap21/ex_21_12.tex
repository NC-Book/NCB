...
Because the maximum node degree is 2, the eigenvalues should be $-2\leq \lambda \leq 2$, so the square root term is imaginary. The equation we found for $b$ almost looks a bit too perfect. Let's compute $|b|$. 
...
$x+iy$, but also also as $r\exp{i\theta}$. In general these two ways of writing the complex number are related via 
\eqa{
x&=& r \cos(\theta) \\
y&=& r \sin(\theta)
}
...
\subquestion At the beginning and end of the chain, the eigenvector condition is slightly different. For example in node 1: $v_2=\lambda v_1$. However, we can write it in the familiar form $v_0+v_2=\lambda v_1$, if we demand $v_0=0$. Hence introduce \textit{ghost nodes}, numbered 0 and $N+1$ and demand that $v_0=v_{N+1}=0$. Use this boundary condition to find all eigenvalues and eigenvectors. 
