...
\solution 
\paragraph{Initial condition $v(0)=0$, $x(0)=1$.}With the parameters given, the eigenvalues are 
\eqa{
...
\eq{
  \cos(at) = \frac{\exp{iat}+\exp{-iat}}{2} \qqq \sin(at) = \frac{\exp{iat}-\exp{-iat}}{2i}
}
...
\paragraph{Initial condition $v(0)=1$, $x(0)=0$.} With the alternative initial conditions this calculation becomes a bit easier. Our equation for the expansion coefficients now reads
\eq{
...
and hence
\eq{
c_1 =- \frac{1}{6}i  \qqq c_2 =+ \frac{1}{6}i.    
}
So, the full solution is 
\eq{
\avec{x(t)\\ v(t) } = \left(-\frac{1}{6}i\right)\avec{1 \\ -1+3i} \exp{(-1+3i)t} + \left(\frac{1}{6}i\right)\avec{1\\ -1-3i} \exp{(-1-3i)t}. 
}
Written in the form of the trigonometric functions:
\eq{
x(t)= \exp{-t}\frac{1}{3}\sin(3t),    
}
\eq{
v(t)= \exp{-t}\left(\cos(3t)-\frac{1}{3}\sin(3t)\right).   
}
\subquestion
