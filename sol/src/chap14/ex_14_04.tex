...
Compute the steady states of the following systems and analyze their stability:
...
where $A$ and $B$ are placeholders for the eigenvector elements. Because we can rescale eigenvectors we can set $A=1$ (unless $A$ needs to be 0). Reading the two rows of the equation system separately gives us 
\eqa{
-2 + 5 B &=& \lambda \\
-1 &=& \lambda B
}
The second row tells us $B=-1/\lambda$, and substituting this into the first row gives us 
\eq{
...
\eq{
{\rm Re}(\lambda_{1,2}) = {\rm Re}(-1 \pm \sqrt{-4}) = -1
}
...
J_{21} &=& \left.\frac{\partial \dot{y}}{\partial x}\right|_* = \left.\frac{\partial}{\partial x} x-3y+8 \right|_* = 1 \\
J_{22} &=& \left.\frac{\partial \dot{y}}{\partial x}\right|_* = \left.\frac{\partial}{\partial y} x-3y+8 \right|_* = -3
}
...
\eq{
{\rm Re}(\lambda_1) = \lambda_1 = -1 + \sqrt{3}
}
Because $\sqrt{3}>1$ this eigenvalue is positive which reveals that the steady state is unstable. 
