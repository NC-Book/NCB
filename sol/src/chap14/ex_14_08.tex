...
\subquestion Find a stability condition for fixed points in 1-dimensional maps. [Follow the procedure, guessing won't work.]
\solution  
...
We can now see that the deviation $\delta$ gets smaller and smaller $|f'(x^*)|<1$, whereas for $|f'(x^*)|>1$ it becomes larger and larger in magnitude. So $|f'(x^*)|<1$ is the stability condition that we are looking for.  

\subquestion
Now, find the stability condition for multi-dimensional maps $\vec{x_{i+1}}=f(\vec{x_i})$, where $\vec{x_i}$ is a vector of dimension $N$.
...
By substituting $\vec{x} = \vec{x^*} + \vec{\delta} $ into the iteration rule we find 
\eq{
...
Here it is again time for the best trick ever: As the eigenvectors form a basis the only possible solution is that the first term on the left matches the first term on the right and so on. Hence,
\eq{
...
\subquestion 
Suppose you wanted to solve $5x=x^2$ by the fixed-point iteration $x_{i+1}={x_i}^2/5$. Do you see why this is problematic? Can you fix it?
...
\eq{x_{i+1}=\sqrt{5x_i}.} %but no, now $-5$ isn't even a fixed point anymore. Taking square roots of things can make solutions vanish, so let's try one more
%\eq{
%x_{i+1} = -\sqrt{-5x_i}
%}
When we evaluate the derivative, we find 
\eq{
