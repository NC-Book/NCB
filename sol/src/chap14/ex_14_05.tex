...
\exercise{Complex system step-by-step}{3}
...
\paragraph{Jacobian.}We compute the elements of the Jacobian matrix.
\eqa{
...
\paragraph{Eigenvalues.}We find the characteristic polynomial using the approach 
\eqn{
...
From the second condition we get $B=-1/\lambda$. Substituting this into the first condition yields 
\eqn{
...
\subquestion 
Determine for which values of $a$ and $b$ the eigenvalues are complex.
...
\subquestion
How does stability depend on $a$ and $b$ if the eigenvalues are complex?
...
\subquestion 
How does stability depend on $a$ and $b$ if the eigenvalues are real?
\solution 
...
Note that squaring both sides of an inequality can produce wrong results if the sides have different signs. However in this case we know the square root is positive and we have already determined that we need $a$ to be positive. Hence if the eigenvalues are real the condition for stability is 
\eqn{a>0,\,b>0}

\subquestion 
Draw a diagram where $a$ and $b$ are the axis. Use lines to indicate where stability changes or eigenvalues become complex. Color in the area where the steady state is stable. 
...
\end{itemize}
We can graphically depict these cases follows:

\begin{center}
\includegraphics[width=0.6\textwidth]{ex14_5}
\end{center}
The steady state is stable in the green (top-right) quadrant, which is borderd by the two stability conditions (strong lines). The brighter area (parabola) correpsonds to the parameter region where the condition for complex eigenvalues is relevant. 
