...
Find the steady states of the following one-dimensional systems and determine their stability:
\subquestion
\eqn{\dot{x}=5-3x}
...
\solution
This equation is already factorized, we could multiply it out, which would give us the warm fuzzy feeling of applying a well-known procedure, but would take us actually farther away from the solution. As it is we can just read off the steady states. They are 
\eqn{
X_1^* = -5,\quad X_2^* = 1,\quad X_3^* = 3,\quad X_4^* = 9.
}
...
\lambda &=& \left. \frac{\partial}{\partial x} (3-x)(x+5)(x-9)(x-1) \right|_{x=-5} \\
   &=& \left. \left( (x+5) \frac{\partial}{\partial x} (3-x)(x-9)(x-1) \right)\right|_{x=-5} \\
   &=&\left.   \left( (3-x)(x-9)(x-1) \frac{\partial}{\partial x} (x+5) \right)\right|_{x=-5} 
}
...
The right-hand side is then negative until it crosses zero again in the steady state at 3 where it becomes positive again. We can see that the slope at this point must be positive so 3 is an unstable steady state. At 9 we cross for a final time, becoming negative again, so the slope in 9 must be negative, marking it as a stable steady state. 
...
\solution
In this case we get a steady state wherever the sin function becomes zero. So, the steady states are
\eq{
x_n^* = n\pi 
}
where $n$ is any integer. 

To evaluate the stability we consider the derivative in the steady states which is $\cos(n\pi)$. So, this derivative is positive if $n$ is even and negative if $n$ is odd. Hence, the steady states corresponding to odd values of $n$ are stable and steady states corresponding to even values of $n$ are unstable. 
