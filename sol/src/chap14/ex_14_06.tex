...
\solution
We start with 
\eq{0=\frac{x^2}{1+x^2}-\frac{m}{2}x}
...
These branches of steady states are interesting because they are only real solutions if $m\le 1$. As we increase $m$ beyond 1 the steady states become complex. Unlike complex eigenvalues which combine to give us oscillations, complex steady states are unreachable for a system that starts out real. 

\subquestion 
Determine the stability of the steady states. (This requires a bit more mathematical technique than most exercises in this book. Give it a try. If you get stuck in the roots check the solution.)
...
where we multiplied by $m^4$, because
\eq{ x_{2}^* =\frac{1 + \sqrt{1-m^2}}{m} }
so the square of the steady state value is 
\eq{
\left(x_{2}^*\right)^2=\frac{1 + 2\sqrt{1-m^2}+ 1 -m^2 }{m^2} = \frac{2 + 2\sqrt{1-m^2} -m^2 }{m^2}
}
so now when we substitute these into the condition we get 
\eqa{
...
Alternatively, we could have used the idea from Ex.~1b to arrive at the results for the second and third state without any calculation. However, in this case I have decided to include the calculations here since they provide a nice example of how to deal with a somewhat messier case.  

\subquestion
Draw a bifurcation diagram: Plot the $x^*$ over $m$ for $0.5<m<1.5$. Indicate the stability of the steady states in the diagram. Also color the area between the branches of steady states according to whether the population size $x$ is increasing or decreasing.   

\solution
\begin{center}
\includegraphics[width=0.6\textwidth]{ex14_6}
\end{center}
The bifurcation diagram reveals a tipping point at $m=1$. If we increase the mortality beyond this point the population will collapse to the state $x_1^*=0$.
