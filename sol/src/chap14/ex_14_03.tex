

\exercise{Stability of the SIS system}{2}
Compute the non-trivial steady state of the SIS model
\eqn{\dot{I}=p(N-I)I-rI}
and determine its stability.

\solution 
We already know that the non-trivial steady state is at 
\eq{
I^* = N-\frac{r}{p}
}
We now compute the derivative 
\eq{
\lambda = \left. \frac{\partial {\dot{I}}}{\partial I} \right|_* = pN-2pI^*-r. 
}
Substituting the steady state we find 
\eqa{
\lambda &=& pN - 2p (N-r/p) -r  \\
        &=& -pN + r
}
So the non-trivial steady state is stable when $r>pN$ and unstable otherwise. So, the non-tirivial state is stable when the trivial steady state is unstable and, vice versa. At $r=pN$ the two states meet and exchange their stability. 
