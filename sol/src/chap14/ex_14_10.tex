...
\mbox{A} &\xrightarrow{v=1}& \oslash
}
Here, A describes the amount of available energy. Energy is needed to start a process (first reaction), which then produces more energy (second reaction), which is used to sustain the organism (third reaction).

\subquestion 
Let $a$ be the amount of A and $b$ the amount of B in the system. Then, write the ODE system that captures the dynamics of these variables, and show that there's no steady state in which $a^*>0$ except in a special case. 
...
This is a linear system and every linear system only has exactly one steady state where all variables are zero. To show this explicitly, we compute the steady state. From the second equation, we get the steady state condition
\eq{
b=\frac{r}{s}a 
}
...
\subquestion 
The absence of a robust steady state where we have a constant level of stored energy is a problem. Let's assume that $r=a^p$, where $p\neq0$ is a new parameter. Show that there is now a steady state with $a^*>0$.
...
for $p>0$ the expression under the square root is positive. Hence, the eigenvalues are real. Thus, the system is stable if (and only if)
\eq{
\frac{p+2+s}{2} > \sqrt{\left(\frac{p+2+s}{2}\right)^2 +sp }
}
For $p>0$ this condition is always false. If you don't see this immediately consider that both sides of the inequality are positive for $p>0$, so we can square both sides
\eqa{
...
So $p$ needs to be negative for a stable steady state in which the cell has a positive supply of energy. That means the enzyme that catalyzes the A $\rightarrow$ B step must act more slowly if there is a lot of A around. In biochemical terms, we can say it is inhibited by its own substrate. This `substrate inhibition' is quite rare in enzymes, but our model suggests that substrate inhibition is needed to keep glycolysis stable. Analyzing a simple model has given us a biological hypothesis that could be tested in experiments. If we did, we would find it to be true (with $p\approx -1$)
