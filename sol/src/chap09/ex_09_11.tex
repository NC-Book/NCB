...
\solution 
\paragraph{Relation between $c_k$ and $s_k$.}First, let's ask ourselves why are $c_k$ and $s_k$ different at all?
...
\paragraph{Equation test on network from Ex.~9.5.}Now that we have an equation we can test it. The simplest test is offered by the network from Ex.~9.5, where every node has degree 1. In that exercise we found 
\eq{
C=x^2. 
}
...
\paragraph{Equation test on network from Ex.~9.6.}Having tested our equation on the simple test example we can now move on to the network from Ex.~9.6. In this case we have 
\eq{
C=\frac{x}{(2-x)^2}.
}
