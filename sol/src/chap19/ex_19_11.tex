...
A dynamical system resides in a state $\vec{x}^*$. Your goal is to reduce the value of the first variable in the steady state, $x_1^*$. You can add a small constant term $\epsilon$ to one of the functions $f$ on the right-hand side of your equation of motion. You recall the implicit function theorem 
\eqn{
...
We can proceed in the same way for the left eigenvectors
\eqa{
...
\subquestion 
Now determine how the first coordinate of steady state $x_1^*$ changes as we changes when we add a small constant term $\epsilon$ to $x_j$'s equation of motion  $f_j(\vec{x})$. Does this give you an idea for another centrality?  
...
where $e_j$ is the $j$th coordinate vector, i.e.~the vector that is zero except for a 1 in element $j$. What happens when we multiply this vector with another vector is that only the $j$th element from the other vector is left. 
...
Let's define the shift of the steady state
\eq{
...
so this is a kind of press perturbation sensitivity. Similar to our other centralities we can exchange $\vec{w}$ and $\vec{v}$ in the equation if we want an indicator for sensitivity to press perturbations instead. 
