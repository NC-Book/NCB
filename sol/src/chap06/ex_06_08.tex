...
\solution
\paragraph{Random employee's average number of friends.}Let's first consider the social network in the company. We can write the degree distribution as  
\eq{
...
\paragraph{Random employee friend's average number of friends.} But, Ada has shared her secret not with a random person, but with a friend. So, let's compute the excess degree distribution 
\eqa{
q_k&=&(k+1)p_{k+1}/z \\
...
We could now also divide the terms by the 7 but this leads to unsightly decimals, so we skip this for now. 
Instead let's compute the expected number of friends that Bob has in addition to Ada. 

This is the mean excess degree 
\eqa{
q&=&\sum k q_k \\
...
So our expectation is that Bob has 37 friends in the company in addition to Ada. Of course this is a statistical average as we know that nobody has 38 friends in the company, but if you look close at our calculation of the excess degree you seem that the the probability that a random friend is one of the people who have 71 friends in the company is slightly more than 3.55/7, so more than 50\%. 

\paragraph{Summery.} We can now answer the question. After Bob has shared Ada's secret with all his friends the people who know it are Bob, Ada and Bob's expected 37 other friends. So, 39 people in total. By contrast if Ada had shared her secret with Chris (a random person at the party), who then had shared it with all his friends, the number would be lower. We only expect a random person to have 7 friends in the company so in this case 1+1+7=9 people would know the secret. This outcome illustrates the key message from the chapter: Your friends are not typical people, but (in average) better connected. (Also: If you have friends like Bob you don't need enemies.)