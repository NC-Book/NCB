...
%(Hint, you might need symbols of the form $[ABCD]$, density of four node chains with states A,B,C,and D, and $[^BA^C_D]$, density of nodes in state A with neighbors in state B, C and D.)  

\solution
To make sure we don't forget any terms, let's go through the processes that occur systematically. The rewiring process occurs at a total rate $w[SI]$ the probability that a rewiring event will destroy an ISI-chain is $2[ISI]/[SI]$. Rewiring cannot create ISI-chains, so the total impact of rewiring is 
\eq{
...
The most complex process is infection. Infection events happen at the total rate $p[SI]$. An infection can destroy ISI-chains if the SI-link on which it occurs is part of an ISI-chain. The probability of this happening is $2[ISI]/[SI]$ (factor of 2 because the ISI contains two $[SI]$-links). It destroys an additional SSI chain if the SI-link was actually part of a star with an S in the center and 3 peripheral nodes (probability $3[^IS_I^I]/[SI]$). 
