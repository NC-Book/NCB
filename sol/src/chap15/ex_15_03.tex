\exercise{Individuality}{2}
Consider a network of agents, who can be in either of two states A, or B. No agent wants to be in the same state as its neighbors. Links in the network are checked at rate $r$. When a link is checked, the agents connected by the link compare their state. If they are in the same state, one of them, chosen randomly, switches to the other state. For example if an AA-link is checked one of the agents would switch to state B. Write a mean field model, find its steady state and check the stability. 
...
we have to compute the Jacobian, which in this case is a 1$\times$1-matrix ($[B]$ acts as an 
