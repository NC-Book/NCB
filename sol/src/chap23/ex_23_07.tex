...
Note that if $p>2r$ then $x_2>x_1$ so in a sense $x_1$ becomes a bottleneck between the highly infected state 2 and the disease free state 1, so for $p>2r$ there can be an outbreak in miniature. If you will, this is a shadow of the epidemic threshold that only really exists in much larger systems. 
...
As a quick check, consider that for $a=0$ we have $x_0=1$. That is the absorbing state coming back. Moreover, for $r=0$ we have $x_0=0$ which makes sense; if we don't recover, there is no way to go back to the disease free state. In the limit of very large recovery rates $r\to \infty$ we also approach the state $X_0 = 1$. 
...
\solution
Due to the symmetry all states with one infected agent behave exactly the same. We thus have 4 microstates, and we again use $x_i$ to denote the probability that there are exactly $i$ infected in the stationary state.

In each transition the number of infected will only ever change by one. Therefore, even on the triangle network, the microstates of the system form a chain, (0)-(1)-(2)-(3).

Since this is a chain, there is only one spanning tree, and I will use $w_i$ denote the weight of this tree when it is oriented such that it points at node $i$. 

We start with the spanning tree directed at microstate 0. In this case all edges need to be oriented such that they point towards states with fewer infected, i.e.~the recovery direction. In state (1) there is only one infected individual so recoveries occur at rate $r$ and this is also the weight of the edge pointing from 1 to 0. In state 2 there are two infected individuals that can recover, so the recovery edge has weight $2r$, and in state 3 there are three, so an edge weight of $3r$. Hence the spanning tree directed at state 0 has the weight
\eq{
w_0 = (r)(2r)(3r) = 6r^3
}
...
\eq{
w_0 = (r)(2r)(3r)\cdots(Nr) = \prod_{n=1}^{N} nr = r^N (N!)
}
...
&=& a p^{i-1}r^{N-i} \left(\prod_{n=1}^{i-1}n(N-n) \right)\left(\prod_{n=i+1}^{N} n \right)  \\
&=& a p^{i-1}r^{N-i} \left((i-1)!\frac{(N-1)!}{(N-i)!} \right)\left(\frac{N!}{i!}\right)\\
&=& a p^{i-1}r^{N-i} \frac{(i-1)!(N-1)!N! }{(N-i)!i!}\\
&=& a p^{i-1}r^{N-i}(i-1)!(N-1)!B(N,i)    
}
...
\eq{
x_1 = A a p^{i-1}r^{N-i}(i-1)!(N-1)!B(N,i)
}
where $A$ is the normalizing factor
\eq{
A = \left((N!)r^N + a\sum_{i=1}^N p^{i-1}r^{N-i}(i-1)!(N-1)!B(N,i)  \right)^{-1}
}