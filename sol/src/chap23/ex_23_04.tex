...
\solution 
We define $x_i$ as the probability that there are $i$ customers in the shop starting from $i=0$. In principle, there is now an infinity of microstates, which makes using Kirchhoff's approach complicated. However, since the number of customers only changes by one in each event, the microstates form a chain, (0)-(1)-..., so that we can use detailed balance. 

Let's start by considering the link between 0 and 1 customers in the shop. If there is just one customer in the shop, they will leave at rate $b$. If there are no customers in the shop, one will arrive at rate $a$. Hence, the detailed balance condition between states 0 and 1 is 
\eq{
ax_0  = bx_1
}
and hence 
...
x_n &=& \frac{a}{nb} x_{n-1}  \\
    ...
    &=& \left(\prod_{m=1}^n \frac{a}{mb} \right) x_0 \\
    &=& \frac{a^n}{b^n n!}  x_0
}
Let's define
\eq{
...
We check
\eq{
1 = \sum_{n=0}^\infty \frac{r^n}{n!} x_0 = x_0 \sum_{n=0}^\infty \frac{r^n}{n!} = x_0 \exp{r}
}
