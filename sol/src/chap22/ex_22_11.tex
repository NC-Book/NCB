...
\eqn{
{\bf J}_{ni,mj} = \left. \frac{\partial \dot{x}_{ni}}{\partial x_{mj}} \right|_*
}
...
\eqa{
 ...
 &=& c_n\sum_k L_{ik} \delta_{nm} \delta_{jk} \\  
 &=& c_n\delta_{nm} L_{ij} \\  
}
...
Now we do this step backwards. The first term contains a $\delta_{ij}$ which describes a matrix that has ones for $i=j$ and zeros elsewhere, i.e. the identity matrix $\bf I$, whereas the second term contains the elements of the Laplacian, so stepping from the blockwise to the system level we can write 
\eq{
