...
We can now identify the elements of the Jacobian of the non-spatial system 
\eqa{
...
We now want to write this equation as 
\eq{
...
In both cases the key is to separate the spaces, and $\otimes$ is our separator that achieves this. In continuous space normal scalar multiplication works as a separator, while in the \emph{network on network} case we need the Kronecker product. In the continuous space the diffusion operator is $\Laplace$ while in the discretized network space it is $-{\bf L}$. Likewise, the identity in the network operator is the identity matrix, while in the continuous space it is 1, which is actually a constant function that is 1 independently of $l$. 
...
The Jacobian in this case is not a matrix because it also contains the differential operator $\Laplace$ and hence also its eigen-objects will not be pure vectors but vectors of functions. But this is all hidden beneath our notation now and we can use the same ansatz $\vec{v}=\vec{a}\otimes\vec{b}$ as before. Where just like in the network-on-network case, we want $\vec{b}$ to live in the space of local dynamics, meaning it will be a vector independent of $l$, whereas $\vec{a}$ lives in the geographical space, so in this case it is scalar function of $l$ (not a vector). Because we changed our definition of $\otimes$, let's rederive the master stability function approach carefully 
\eqa{
...
In the first step we used that $\otimes$ is now just the normal product and we can move the factors in the product around unless we change the order of matrices or move a spatial function past a differential operator. So, the spatial function $\vec{a}$ cannot move past $\Laplace$ and the vector $\vec{b}$ cannot move past the matrices $\bf P$ and $\bf C$, but we can otherwise freely move the terms around in their respective products.

In the second step we used ${\bf I}\vec{a}= 1\vec{a} = \vec{a}$ and assumed that $\vec{a}$ is an eigenfunction of $\Laplace$ with the eigenvalue $\kappa$. Finally, in the fourth step we assume that $\vec{b}$ is an eigenvector of ${\bf P}+\kappa {\bf C}$ with eigenvalue $\lambda$. 
...
So we are looking for a function that when differentiated twice is equal to itself times a factor $\kappa$. Moreover, we want our function to obey the boundary conditions, that is $a(0)=a(1)=0$. The only functions that can obey these constraints are
\eq{
a(l)=\sin(n\pi l)
}
