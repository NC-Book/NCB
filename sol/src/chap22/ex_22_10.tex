...
Demand that the system has a Turing bifurcation at $\kappa=2.5$ and another one at $\kappa=3.5$. Use the resulting conditions to express $d$ and $bc$ as a function of $a$
...
These two conditions almost have an identical form, so let's subtract one from the other. The result is 
\eq{
...
which is what we wanted: The system is unstable for $\kappa=3$. (Essentially these checks work out as intended because we choose the diffusion rates differently enough, and because specifying two zeros of the determinant prevents further zeros from appearing as it is a quadratic function of $\kappa$.)

The only additional constraint that we discovered is $a>60/9$, so to have a nice value I will use $a=10$. We can now compute 
\eq{
d=60-10a=-40 
}
and 
\eq{
bc=-10a^2+60a-87.5=-1000+600-87.5=-487.5
}
...
\solution 
If we are willing to cut some corners this can be extremely simple. For example, the question did not specify that the steady state can't be at zero. So we can just use a linear system,
\eqa{
\dot{x}&=&10x+10y \\
\dot{y}&=&-48.75x -40y 
}
...
Clearly $y=x=1$ is a steady state and the corresponding Jacobian is $\bf P$. While computing steady states is hard, designing the system so that it has a specific steady state is quite simple. But while this second system technically solves the question the large exponents would make it almost impossible to simulate on a network. 
...
Clearly our value of $P_{12}=10$ is not happening but we only need $P_{12}$ and $P_{21}$ to multiply to -487.5 so with $P_{12}=-20$ we now need $P_{21}= 24.375$. We consider the second equation 
\eq{
...
\eq{
\dot{y} = \frac{97.5 xy}{1 +x} - (48.75-r_5) y^2-r_5y. 
}
Let's check the final derivative 
\eqa{
{\bf P_{22}} &=& \left. \frac{\partial \dot{y}}{\partial y} \right|_* \\
 &=& \left. \frac{\partial}{\partial y} \frac{97.5 xy}{1 +x} - (48.75-r_5) y^2-r_5y \right|_* \\
  &=& \left. \frac{97.5 x}{1 +x} - 2(48.75-r_5) y-r_5 \right|_* \\
  &=&  \frac{97.5}{2} - 2(48.75-r_5)-r_5  \\
  &=& - 48.75+2r_5-r_5  \\
  &=& - 48.75+r_5  \\
}
We wanted a value of $P_{22}=-40$ so 
\eq{
r_5=8.75
}
and hence 
...
\eqa{
\dot{x} &=& 20x - \frac{40 xy}{1+x} \\ 
\dot{y} &=& \frac{97.5 xy}{1 +x} -40 y^2-8.75y
}
