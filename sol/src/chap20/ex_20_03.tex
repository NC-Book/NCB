...
So, this is ${\bf A}\vec{v}$. If $\vec{v}$ is an eigenvector, then this should be the same as $\lambda \vec{v}$, which yields the condition
\eq{
...
\eq{
\mu_1 = 1, \quad \mu_2 = 1, \quad \mu_3=-1, \quad \mu_4=-1.
}
...
The network is bipartite, which means that the three missing eigenvalues must be 0 and a symmetric pair. Let's say the symmetric eigenvalues are $m$ and $-m$. We know that the squares of all eigenvalues must add up to two times the number of links, so 
\eq{
