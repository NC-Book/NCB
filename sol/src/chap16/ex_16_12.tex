...
Let's return to the system from Ex.~16.3. Could you use the approach from the implicit function theorem also to compute $x''(p)$ at $p=1$? How about $x'''(p)$ and $x''''(p)$? Use this to construct $x^*(p)$ as a Taylor expansion. Use your result to find $x^*(0)$, and verify that it is a steady state.    
...
Therefore $x'=2$ at $p=1$, but we already knew this. Let's take a look at Eq.~\eqref{eqSes2} instead 
\eqa{
...
We know from the Ex.~16.3 that $x(1)=2$ and computed here and previously that $x'(1)=2$. there we have 
\eq{
x(p) = 2+ 2(p-1)
}
