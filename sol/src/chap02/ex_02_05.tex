...
\subquestion Given the distances above and the link list from the previous exercise, is it possible that we reach Tewen from Isangel?
...
\subquestion So, on the shortest path from Fort Kerron, which place do I visit directly before arriving in Tewen?
...
\subquestion And how do I get to the place that is the answer (b)? Directly from Fort Kerron? Or, do I need to visit a different place on the way?
...
\subquestion Formulate a general strategy to find the the shortest path given the link list and the precomputed distances from the start point to all places.  

\solution
Based on the insights from the previous parts we can now formulate the following algorithm:
\paragraph{Pathfinding.} Given a a list of distances $\vec{d}=d_1, \ldots, d_N $ from a starting point $S$ and the direct distances $D_{ij}$ for all direct routes from $j$ to $i$. We can find the shortest path from $S$ to a destination $Z$ as follows:
